...
`/new` 启动一个全新的 Claude Code 会话，只携带真正需要的上下文——不带完整对话历史的包袱。
...
每个会话默认拥有独立的 git worktree，并行工作不会冲突。
...
两个 skill 共用一个 bash 脚本（`scripts/claude-fork.sh`），负责 iTerm2 pane 创建（通过 AppleScript）、worktree 设置、plan mode 标志以及孤立 symlink 的清理。
